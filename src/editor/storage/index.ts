import { AnxiEvent } from '../../ts/aixi/eventer/Event';
import { Storager } from './core';
/**
 * 基于localStorage实现的实时缓存系统, 5MB之内的缓存存储时间基本小于一帧时间，不会造成UI线程卡死
 */
export class ImmediateStorager<T extends { [key: string]: any }> extends Storager {
    constructor(name: string, defaultObjs: T) {
        super(name);
        const self = this;
        this.name = name;
        let obj: T;
        ...
        let handler = <ProxyHandler<any> & { name: string }>{
            ...
            defineProperty(target, p: string, descriptor) {
                ...
                self.emit(new AnxiEvent(`change${this.name + '/' + p}`, descriptor.value));
                return res;
            },
            deleteProperty(target, property: string) {
                ...
                self.emit(new AnxiEvent(`change${this.name + '/' + property}`, undefined));
                return res;
            }
        };
